"""Bull Researcher - Optimistic analyst that finds buy arguments."""
...
class BullResearcher:
    """Bullish researcher - finds buy arguments."""

    analyst_type = "bull_researcher"

    def __init__(self, llm_client: BaseLLMClient):
        """Initialize BullResearcher.

        Args:
            llm_client: LLM client for generating analysis.
        """
        self.llm = llm_client
        self.system_prompt = """You are Bull Researcher - an optimistic analyst.
Your job is to find compelling BUY arguments for a stock.
You MUST:
- Find at least 3 bull points with specific numbers/prices
- Predict Bear's counterarguments and pre-emptively rebut them
- Focus on: valuation, catalysts, growth drivers, competitive advantages"""
    ...
    def run(
        ...
        Returns:
            dict with 'bull_points' (list) and 'content' (str).
        """
        ...
        # Build context from debate history if available
        debate_context = self._build_debate_context(debate_history)

        prompt = self._build_bull_prompt(symbol, reports_summary, debate_context)

        response = self.llm.generate(prompt, temperature=0.7, max_tokens=4096)

        bull_points = self._extract_bull_points(response.content)
    ...
    def _build_debate_context(self, debate_history: list) -> str:
        ...
        for i, round_data in enumerate(debate_history[-2:], 1):
            bear_content = round_data.get("bear", {}).get("content", "")
            context_parts.append(f"Previous Bear argument: {bear_content}")
    ...
    def _build_bull_prompt(
        ...
        """Build prompt for bull analysis.
        ...
        """
        prompt_content = f"""You are Bull Researcher - an optimistic analyst analyzing {symbol}.
...
Your task:
1. Find at least 3 compelling BUY arguments with specific numbers/prices
2. Pre-emptively rebut potential Bear counterarguments
3. Focus on: valuation opportunities, catalysts, growth drivers, competitive advantages

Return your analysis in the following JSON format:
```json
{{
    "bull_points": ["Point 1 with specific numbers", "Point 2 with specific numbers", "Point 3 with specific numbers"],
    "content": "Your detailed bull case analysis"
}}
```
"""
    ...
    def _extract_bull_points(self, text: str) -> list:
        """Extract bull points from LLM response.
